...
class _PersonWidgetBase(object):
    ...
    def drag_data_get(self, widget, context, sel_data, info, time):
        ...
        if sel_data.target == DdTargets.PERSON_LINK.drag_type:
            ...
            sel_data.set(
                sel_data.target, 8, self.format_helper.format_person(self.person, 11)
            )
...
"""
    ...
    def Tree_Rebuild(self):
        ...
        LstDescendants = self.Tree_Find_Relatives(
            layout_widget, person, 0, generations[0], 1
        )
        LstAncestors = self.Tree_Find_Relatives(
            layout_widget, person, 0, generations[1], -1, LstDescendants[1]
        )
        ...
        if True:  # Draw Border of the layout for debugging
            ...
            self.gtklayout_lines.append(
                [RequiredWidth - 1, RequiredHeight - 1, RequiredWidth - 1, 1]
            )
            self.gtklayout_lines.append(
                [RequiredWidth - 1, RequiredHeight - 1, 1, RequiredHeight - 1]
            )

        # Move boxes to their desired position, draw connection lines, etc.
        self.Tree_MoveBranchBoxes(
            layout_widget, LstAncestors, ActivePersonX, A_Top, -1, 0
        )
        self.Tree_MoveBranchBoxes(
            layout_widget, LstDescendants, ActivePersonX, D_Top, 1, 0
        )
        ...
        if self.use_timeline and self.Tree_EstimateBirth(LstDescendants[0]):
            ...
            Pos50 = (
                ActivePersonX
                + (
                    self.Tree_EstimateBirth(LstDescendants[0])
            ...
            for Tick in Ticks:
                if Tick[1] > 0 and Tick[1] < RequiredWidth:
                    self.gtklayout_lines.append(
                        [
                            Tick[1],
                            int(5 * TimeLineHeight / 8),
                            Tick[1],
                            int(7 * TimeLineHeight / 8),
                            1,
                        ]
                    )
                    if Tick[0]:
                        ...
                        layout_widget.put(
                            label,
                            int(Tick[1] - label.get_preferred_size()[0].width / 2),
                            1 * TimeLineHeight / 4,
                        )
    ...
    def Tree_MoveBranchBoxes(
        self, layout_widget, BranchData, BoxRight, BranchTop, Direction, genDepth
    ):
        """
        ...
        if False:  # draw Branch-Border for debugging
            self.gtklayout_lines.append(
                [
                    BoxRight,
                    BranchTop + 1,
                    BoxRight - Direction * BranchData[2][0],
                    BranchTop + 1,
                ]
            )
            self.gtklayout_lines.append(
        ...
        if (
            self.use_timeline
            and self.show_lifespan
            and BranchData[0]
            and not (Direction < 0 and genDepth == 0)
        ):
            ...
            self.gtklayout_boxes.append(
                [
                    xBox - lifespan * 11 + pbwSize.width,
                    yBox,
                    xBox + 5,
                    yBox + pbwSize.height,
                    color,
                ]
            )  # +5 for overlapping with the box
        ...
        xvline = (
            xBoxConnection - Direction * DistX / 2
        )  # default for descendants and if date of marriage not known
        if self.use_timeline and Direction < 0 and BranchData[0]:
            ...
            if family:
                ...
                if marriagedate:
                    ...
                    if (
                        bDate is not None
                        and not bDate.is_empty()
                        and mDate is not None
                        and not mDate.is_empty()
                    ):
                        timespan = (
                            bDate.to_calendar("gregorian").get_year()
        ...
        for branch in BranchData[3]:
            ...
            ChildBoxConnection = self.Tree_MoveBranchBoxes(
                layout_widget,
                branch,
                ChildBoxRight,
                ChildBranchTop,
            ...
            self.gtklayout_lines.append(
                [
                    xvline,
                    ChildBoxConnection[1],
                    ChildBoxConnection[0],
                    ChildBoxConnection[1],
                ]
            )
        ...
        if (
            len(BranchData[3]) > 0
        ):  # There are relatives, so Connection lines must be drawn
            self.gtklayout_lines.append(
                [xvline, min(yChildBoxConnection), xvline, max(yChildBoxConnection)]
            )
            self.gtklayout_lines.append(
                [xBoxConnection, yBoxConnection, xvline, yBoxConnection]
            )

        # Direction<0 -> Drawing ancestors: Show Marriage Info
        if (
            Direction < 0
            and len(BranchData[3]) > 0
            and self.show_marriage_data
            and BoxSizes[4] > 0
        ):
            text = " "
            family_handle = None
            ...
            if family_handle:
                ...
                label.connect(
                    "button-press-event", self.family_button_press_cb, family_handle
                )
            label.show()
            ...
            layout_widget.put(
                label,
                xvline + 5,
                int(yBoxConnection - label.get_preferred_size()[0].height / 2),
            )
    ...
    def get_date_marriage(self, family):
        for event_ref in family.get_event_ref_list():
            ...
            if (
                event
                and event.get_type() == gramps.gen.lib.EventType.MARRIAGE
                and (
                    event_ref.get_role() == gramps.gen.lib.EventRoleType.FAMILY
                    or event_ref.get_role() == gramps.gen.lib.EventRoleType.PRIMARY
                )
            ):
                return event
        return None
    ...
    def Tree_Create_PersonBox(self, layout_widget, person, x, y, maxlines, Relative):
        ...
        if person:
            if (
                maxlines > 3 and self.show_images
            ):  # and i < ((len(positions)-1)/2) and  positions[i][0][3] > 1:
        ...
        pbw = PersonBoxWidgetCairo(
            self, self.format_helper, person, alive, maxlines, image
        )
        if maxlines < 7:
    ...
    def Tree_Find_Relatives(
        ...
        if Widget:
            ...
            pbw = self.Tree_Create_PersonBox(
                layout_widget, person, 100, 100, BoxMaxLines, CalledFromPerson
            )
        ...
        if self.show_lifespan and person and birthdate:
            ...
            if death:
                ...
                lifespan = (
                    deathdate.to_calendar("gregorian").get_year()
                    - birthdate.to_calendar("gregorian").get_year()
                )
        ...
        for Relative in RelPersons:
            Ret = self.Tree_Find_Relatives(
                layout_widget, Relative, genDepth + 1, genMax, Direction, None, person
            )
            ...
            yRelConnect.append(
                yRelBranchTop + Ret[2][4] + Ret[1].get_preferred_size()[0].height / 2
            )
        ...
        if len(yRelConnect) > 0:
            yPersonBoxTop = (
                max(yRelConnect) + min(yRelConnect)
            ) / 2 - pbwSize.height / 2
        ...
        if birth:
                    ...
                    for child_ref in family.get_child_ref_list():
                        ...
                        if child is not None:
                            if child.handle not in callerHandleList:
                                childDate = self.Tree_EstimateBirth(
                                    child, callerHandleList
                                )
            ...
            if len(ChildBirthDates) > 0:
                ...
                if family is not None:
                    father_handle = family.get_father_handle()
                    if (
                        father_handle is not None
                        and father_handle not in callerHandleList
                    ):
                        ParentDate = self.Tree_EstimateBirth(
                            self.dbstate.db.get_person_from_handle(father_handle),
                            callerHandleList,
                        )
                    ...
                    mother_handle = family.get_mother_handle()
                    if (
                        mother_handle is not None
                        and mother_handle not in callerHandleList
                    ):
                        ParentDate = self.Tree_EstimateBirth(
                            self.dbstate.db.get_person_from_handle(mother_handle),
                            callerHandleList,
    ...
    def missing_parent_button_press_cb(self, obj, event, person_handle, family_handle):
        ...
        if event.button == 1 and event.type == Gdk.EventType._2BUTTON_PRESS:
            ...
            self.build_missing_parent_nav_menu_cb(
        ...
        if self.dbstate.active:
            ...
            if len(childlist) == 1:
                ...
                for child_handle in childlist:
                    ...
                    menuitem.connect(
                        "activate", self.on_childmenu_changed, child_handle
                    )
                    menuitem.show()
    ...
    def change_show_unknown_peoples_cb(self, event):
        ...
        self.cman.set("interface.show-unknown-people", self.show_unknown_peoples)
        self.dirty = True
        self.Tree_Rebuild()
    ...
    def add_settings_to_menu(self, menu):
        ...
        menu.append(
            self.create_menu_item(
                _("Show images"), self.show_images, self.change_show_images_cb
            )
        )
        ...
        menu.append(
            self.create_menu_item(
                ...
                self.change_show_marriage_cb,
            )
        )

        menu.append(
            self.create_menu_item(
                _("Order by timeline"), self.use_timeline, self.change_use_timeline_cb
            )
        )

        menu.append(
            self.create_menu_item(
                _("Show lifespan"), self.show_lifespan, self.change_show_lifespan_cb
            )
        )
        ...
        for num in range(0, 10):
            entry = Gtk.RadioMenuItem(
                label=ngettext("%d generation", "%d generations", num) % num
            )
    ...
    def build_missing_parent_nav_menu_cb(
    ...
    def build_relation_nav_menu_cb(self, obj, event, family_handle):
        ...
        clipboard_item = Gtk.MenuItem.new_with_mnemonic(_("_Copy"))
        clipboard_item.connect(
            "activate", self.copy_family_to_clipboard_cb, family_handle
        )
        clipboard_item.show()
        self.menu.append(clipboard_item)
